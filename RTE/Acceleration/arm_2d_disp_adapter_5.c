...
#ifndef __DISP5_CFG_ITERATION_CNT__
#   define __DISP5_CFG_ITERATION_CNT__     30
#endif
...
/*============================ PROTOTYPES ====================================*/
extern 
int32_t Disp5_DrawBitmap(int16_t x, 
                        int16_t y, 
                        int16_t width, 
                        int16_t height, 
...
ARM_NOINIT 
arm_2d_scene_player_t DISP5_ADAPTER;

#if __DISP5_CFG_ENABLE_3FB_HELPER_SERVICE__
ARM_NOINIT
static 
...
static struct {
    ...
    .wMin = UINT32_MAX,
    .wIterations = __DISP5_CFG_ITERATION_CNT__,
};
...
#if !__DISP5_CFG_DISABLE_NAVIGATION_LAYER__
static
IMPL_PFB_ON_DRAW(__pfb_draw_navigation)
{
    ...
    if (__DISP5_CFG_ITERATION_CNT__) {
                    ...
                    (arm_2d_region_t []){
                        {
                            .tLocation = {
                                .iX = 0,
                                .iY = ((__DISP5_CFG_SCEEN_HEIGHT__ + 7) / 8 - 2) * 8},
                            .tSize = {
                                .iWidth = __DISP5_CFG_SCEEN_WIDTH__,
                                .iHeight = 8,
                            },
                        },
                    }, 
        ...
        arm_lcd_text_location((__DISP5_CFG_SCEEN_HEIGHT__ + 7) / 8 - 2,
                              0);
...
#if __DISP5_CFG_SCEEN_WIDTH__ >= 240
        arm_lcd_printf( 
...
#if __DISP5_CFG_ENABLE_3FB_HELPER_SERVICE__
__WEAK
IMPL_PFB_ON_LOW_LV_RENDERING(__glcd5_pfb_render_handler)
{
    ...
    if (__arm_2d_helper_3fb_draw_bitmap(&s_tDirectModeHelper,
        ...
        arm_2d_helper_pfb_report_rendering_complete(
                        &DISP5_ADAPTER.use_as__arm_2d_helper_pfb_t);
    }
}

void *disp_adapter5_3fb_get_flush_pointer(void)
{
    return arm_2d_helper_3fb_get_flush_pointer(&s_tDirectModeHelper);
}


#   if __DISP5_CFG_ENABLE_ASYNC_FLUSHING__
...
void disp_adapter5_insert_2d_copy_complete_event_handler(void)
{
    arm_2d_helper_pfb_report_rendering_complete(
                    &DISP5_ADAPTER.use_as__arm_2d_helper_pfb_t);
}
...
void disp_adapter5_insert_dma_copy_complete_event_handler(void)
{
...
void disp_adapter5_insert_async_flushing_complete_event_handler(void)
{
    arm_2d_helper_pfb_report_rendering_complete(
                    &DISP5_ADAPTER.use_as__arm_2d_helper_pfb_t);
}
...
IMPL_PFB_ON_LOW_LV_RENDERING(__glcd5_pfb_render_handler)
{
    ...
    __disp_adapter5_request_async_flushing(
                    pTarget,
                    bIsNewFrame,
...
IMPL_PFB_ON_LOW_LV_RENDERING(__glcd5_pfb_render_handler)
{
    ...
    Disp5_DrawBitmap(ptTile->tRegion.tLocation.iX,
                    ptTile->tRegion.tLocation.iY,
                    ptTile->tRegion.tSize.iWidth,
                    ptTile->tRegion.tSize.iHeight,
...
static bool __on_each_frame_complete(void *ptTarget)
{
...
#if __DISP5_CFG_FPS_CACULATION_MODE__ == ARM_2D_FPS_MODE_REAL
...
#else /* __DISP5_CFG_FPS_CACULATION_MODE__ == ARM_2D_FPS_MODE_RENDER_ONLY */
    int32_t nElapsed = DISP5_ADAPTER.use_as__arm_2d_helper_pfb_t.Statistics.nTotalCycle;
#endif

    int32_t nTotalLCDCycCount = DISP5_ADAPTER.use_as__arm_2d_helper_pfb_t.Statistics.nRenderingCycle;
    ...
    if (__DISP5_CFG_ITERATION_CNT__) {
        if (BENCHMARK.wIterations) {
            ...
            BENCHMARK.dwRenderTotal += DISP5_ADAPTER.use_as__arm_2d_helper_pfb_t.Statistics.nTotalCycle;
            BENCHMARK.wIterations--;

            if (0 == BENCHMARK.wIterations) {
                BENCHMARK.wAverage =
                    (uint32_t)(BENCHMARK.dwTotal / (uint64_t)__DISP5_CFG_ITERATION_CNT__);
                ...
                BENCHMARK.wIterations = __DISP5_CFG_ITERATION_CNT__;
...
{
    memset(&DISP5_ADAPTER, 0, sizeof(DISP5_ADAPTER));

    //! initialise FPB helper
    if (ARM_2D_HELPER_PFB_INIT(
        &DISP5_ADAPTER.use_as__arm_2d_helper_pfb_t,                            //!< FPB Helper object
        __DISP5_CFG_SCEEN_WIDTH__,                                     //!< screen width
        __DISP5_CFG_SCEEN_HEIGHT__,                                    //!< screen height
        COLOUR_INT,                                                             //!< colour date type
        __DISP5_CFG_PFB_BLOCK_WIDTH__,                                 //!< PFB block width
        __DISP5_CFG_PFB_BLOCK_HEIGHT__,                                //!< PFB block height
        __DISP5_CFG_PFB_HEAP_SIZE__                                    //!< number of PFB in the PFB pool

#if     __DISP5_CFG_VIRTUAL_RESOURCE_HELPER__                          \
    &&  !__DISP5_CFG_USE_HEAP_FOR_VIRTUAL_RESOURCE_HELPER__
        + 3
#endif
        ,{
            .evtOnLowLevelRendering = {
                //! callback for low level rendering
                .fnHandler = &__glcd5_pfb_render_handler,
            },
            .evtOnEachFrameCPL = {
                .fnHandler = &__on_each_frame_complete,
            },
        },
#if __DISP5_CFG_SWAP_RGB16_HIGH_AND_LOW_BYTES__
        .FrameBuffer.bSwapRGB16 = true,
#endif
#if __DISP5_CFG_DEBUG_DIRTY_REGIONS__
        .FrameBuffer.bDebugDirtyRegions = true,
#endif
        .FrameBuffer.u3PixelWidthAlign = __DISP5_CFG_PFB_PIXEL_ALIGN_WIDTH__,
        .FrameBuffer.u3PixelHeightAlign = __DISP5_CFG_PFB_PIXEL_ALIGN_HEIGHT__,
#if     __DISP5_CFG_VIRTUAL_RESOURCE_HELPER__                          \
    &&  !__DISP5_CFG_USE_HEAP_FOR_VIRTUAL_RESOURCE_HELPER__
...
#if __DISP5_CFG_ENABLE_3FB_HELPER_SERVICE__
    do {
    extern uintptr_t __DISP_ADAPTER5_3FB_FB0_ADDRESS__;
    extern uintptr_t __DISP_ADAPTER5_3FB_FB1_ADDRESS__;
    extern uintptr_t __DISP_ADAPTER5_3FB_FB2_ADDRESS__;
    
    extern arm_2d_helper_2d_copy_handler_t __disp_adapter5_request_2d_copy;
    extern arm_2d_helper_dma_copy_handler_t __disp_adapter5_request_dma_copy;
    
    
        arm_2d_helper_3fb_cfg_t tCFG = {
            .tScreenSize = {
                __DISP5_CFG_SCEEN_WIDTH__,
                __DISP5_CFG_SCEEN_HEIGHT__,
            },
            .chPixelBits = __DISP5_CFG_COLOUR_DEPTH__,
            .pnAddress = {
                [0] = ((uintptr_t)__DISP_ADAPTER5_3FB_FB0_ADDRESS__),
                [1] = ((uintptr_t)__DISP_ADAPTER5_3FB_FB1_ADDRESS__),
                [2] = ((uintptr_t)__DISP_ADAPTER5_3FB_FB2_ADDRESS__),
            },
#if __DISP0_CFG_ENABLE_ASYNC_FLUSHING__
            .evtOn2DCopy = {
                .fnHandler = __disp_adapter5_request_2d_copy,
            },
            .evtOnDMACopy = {
                .fnHandler = __disp_adapter5_request_dma_copy,
            },
#endif
        };
    ...
    arm_lcd_text_init((arm_2d_region_t []) {
                        { .tSize = {
                            .iWidth = __DISP5_CFG_SCEEN_WIDTH__,
                            .iHeight = __DISP5_CFG_SCEEN_HEIGHT__,
                        }}});
...
void disp_adapter5_init(void)
{
    ...
    if (!__DISP5_CFG_DISABLE_DEFAULT_SCENE__) {
    #if 0
        ...
        IMPL_ARM_2D_REGION_LIST(s_tDirtyRegions, const static)
            ...
            ADD_LAST_REGION_TO_LIST(s_tDirtyRegions,
                .tLocation = {
                    .iX = ((__DISP5_CFG_SCEEN_WIDTH__ - 100) >> 1),
                    .iY = ((__DISP5_CFG_SCEEN_HEIGHT__ - 100) >> 1),
                },
                .tSize = {
                    .iWidth = 100,
                    .iHeight = 100,
...
arm_fsm_rt_t disp_adapter5_task(void)
{
    return arm_2d_scene_player_task(&DISP5_ADAPTER);
}
...
__WEAK
void * __disp_adapter5_aligned_malloc(size_t nSize, size_t nAlign)
{
...
__WEAK
void __disp_adapter5_free(void *pMem)
{
    if (NULL != pMem) {
        free(pMem);
    }
...
intptr_t __disp_adapter5_vres_asset_loader (
                                            uintptr_t pObj, 
                                            ...
                                            arm_2d_region_t *ptRegion)
{
...
#if __DISP5_CFG_USE_HEAP_FOR_VIRTUAL_RESOURCE_HELPER__
    pBuffer = __disp_adapter5_aligned_malloc(tBufferSize, nPixelSize);
    assert(NULL != pBuffer);
...
#else
    arm_2d_pfb_t *ptPFB = __arm_2d_helper_pfb_new(&DISP5_ADAPTER.use_as__arm_2d_helper_pfb_t);
    assert(NULL != ptPFB);
    ...
    if (nBitsPerPixel < 8) {
        ...
        uintptr_t pSrc = __disp_adapter5_vres_get_asset_address(pObj, ptVRES);
        ...
        for (int_fast16_t y = 0; y < ptRegion->tSize.iHeight; y++) {
            __disp_adapter5_vres_read_memory(   pObj, 
                                                (void *)pDes, 
                                                (uintptr_t)pSrc, 
                                                nBytesPerLine);
        ...
        for (int_fast16_t y = 0; y < ptRegion->tSize.iHeight; y++) {
            __disp_adapter5_vres_read_memory( 
                                            pObj, 
                                            (void *)pDes, 
                                            (uintptr_t)pSrc, 
...
void __disp_adapter5_vres_buffer_deposer (
                                            uintptr_t pTarget, 
                                            arm_2d_vres_t *ptVRES, 
                                            intptr_t pBuffer )
{
    ...
    if ((intptr_t)NULL != pBuffer) {
        __disp_adapter5_free((void *)pBuffer);
    }
#else
    ARM_2D_UNUSED(pTarget);
    ARM_2D_UNUSED(ptVRES);
    ...
    __arm_2d_helper_pfb_free(&DISP5_ADAPTER.use_as__arm_2d_helper_pfb_t, ptPFB);
#endif
}
