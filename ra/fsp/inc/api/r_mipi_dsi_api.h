...
/** MIPI DSI data ID types - See MIPI specification for additional information */
typedef enum e_mipi_dsi_cmd_id
{
    ...
    MIPI_DSI_CMD_ID_GENERIC_SHORT_WRITE         = 0x03,         ///< (Short) Generic Short WRITE, no parameters
    ...
    MIPI_DSI_CMD_ID_DCS_SHORT_WRITE       = 0x05,               ///< (Short) DCS Short WRITE, no parameters
    MIPI_DSI_CMD_ID_DCS_SHORT_WRITE_PARAM = 0x15,               ///< (Short) DCS Short WRITE, 1 parameter
    ...
    MIPI_DSI_CMD_ID_PIXEL_STREAM_3BYTE_18  = 0x2E,              ///< (Long) Loosely Packed Pixel Stream, 18-bit RGB, 6-6-6 Format
...
typedef enum e_mipi_dsi_dcs_id
{
    ...
    MIPI_DSI_DCS_ID_READ_MEMORY_CONTINUE    = 0x3E, ///< Read image data from peripheral, continueing after last read
    ...
    MIPI_DSI_DCS_ID_SET_GAMA_CURVE          = 0x26, ///< Select gamma curve used by display
    ...
    MIPI_DSI_DCS_ID_SET_VSYNC_TIMING        = 0x40, ///< Set VSYNC timingading the specified length of PPS data
    MIPI_DSI_DCS_ID_SOFT_RESET              = 0x01, ///< Software reset
    MIPI_DSI_DCS_ID_WRITE_ACMD              = 0x63, ///< Write access to ACMD registers
    MIPI_DSI_DCS_ID_WRITE_CONTROL_DISPLAY   = 0x53, ///< Wirte control mode of display brightness
    ...
    MIPI_DSI_DCS_ID_WRITE_MEMORY_CONTINUE   = 0x3C, ///< Transfer image information from host to peripheral
...
typedef enum mipi_dsi_video_data
{
...
{
    MIPI_DSI_ACK_ERR_VC_NONE = 0x0,    ///< No Error
    MIPI_DSI_ACK_ERR_VC0     = 0x1,    ///< Virtual channel 0
    MIPI_DSI_ACK_ERR_VC1     = 0x2,    ///< Virtual channel 1
    MIPI_DSI_ACK_ERR_VC2     = 0x4,    ///< Virtual channel 2
    MIPI_DSI_ACK_ERR_VC3     = 0x8,    ///< Virtual channel 3
} mipi_dsi_vc_t;
...
{
    MIPI_DSI_RECEIVE_STATUS_STATUS_NONE        = 0x00000000, ///< Receive status not set
...
typedef enum e_mipi_dsi_fatal_status
{
    ...
    MIPI_DSI_FATAL_STATUS_LPDT_SYNC_ERROR       = 0x00020000, ///< Fatal low power data transmission synchronization
...
/** MIPI DSI physical lane errors */
typedef enum e_mipi_dsi_phy_status
{
    MIPI_DSI_PHY_STATUS_NONE                 = 0x00000000, ///< Physical lane status not set
    MIPI_DSI_PHY_STATUS_ULP_NOT_ACTIVE       = 0x00000001, ///< Physical lane ULP not active status
    MIPI_DSI_PHY_STATUS_CLOCK_LANE_STOP      = 0x00000002, ///< Physical lane clock lane stop state
    MIPI_DSI_PHY_STATUS_DATA_LANE0_LP_RX     = 0x00000004, ///< Physical lane low power data receive mode
    MIPI_DSI_PHY_STATUS_DATA_LANE0_ULP_RX    = 0x00000008, ///< Physical lane ultra low power receive mode
    MIPI_DSI_PHY_STATUS_DATA_LANE0_NOT_ULP   = 0x00000010, ///< Physical lane lane 0 not in ULP mode
    MIPI_DSI_PHY_STATUS_DATA_LANE1_NOT_ULP   = 0x00000020, ///< Physical lane lane 1 not in ULP mode
    MIPI_DSI_PHY_STATUS_DATA_LANE0_STOP      = 0x00000100, ///< Physical lane lane 0 stop state
    MIPI_DSI_PHY_STATUS_DATA_LANE1_STOP      = 0x00000200, ///< Physical lane lane 1 stop state
    MIPI_DSI_PHY_STATUS_DATA_LANE0_RX_TO_TX  = 0x00001000, ///< Physical lane Rx to Tx transition event
    MIPI_DSI_PHY_STATUS_DATA_LANE0_TX_TO_RX  = 0x00002000, ///< Physical lane Tx to Rx transition event
    MIPI_DSI_PHY_STATUS_DATA_LANE0_RX_STATE  = 0x00008000, ///< Physical lane Rx active state
    MIPI_DSI_PHY_STATUS_CLOCK_ULPS_ENTER     = 0x01000000, ///< Physical lane ULPS enter event
    MIPI_DSI_PHY_STATUS_CLOCK_ULPS_EXIT      = 0x02000000, ///< Physical lane ULPS exit event
    MIPI_DSI_PHY_STATUS_CLOCK_LP_TO_HS       = 0x04000000, ///< Physical lane LP to HS transition event
    MIPI_DSI_PHY_STATUS_CLOCK_HS_TO_LP       = 0x08000000, ///< Physical lane HS to LP transition event
    MIPI_DSI_PHY_STATUS_DATA_LANE_ULPS_ENTER = 0x10000000, ///< Physical lane ULPS enter event
    MIPI_DSI_PHY_STATUS_DATA_LANE_ULPS_EXIT  = 0x20000000, ///< Physical lane ULPS exit event
...
typedef enum e_mipi_dsi_link_status
{
    ...
    MIPI_DSI_LINK_STATUS_CH0_RUNNING   = 0x0001, ///< Link status channel 0 running status
    MIPI_DSI_LINK_STATUS_CH1_RUNNING   = 0x0010, ///< Link status channel 1 running status
    MIPI_DSI_LINK_STATUS_VIDEO_RUNNING = 0x0100, ///< Link status video running status
    MIPI_DSI_LINK_STATUS_HP_MODE_BUSY  = 0x1000, ///< Link status HP operation busy status
    MIPI_DSI_LINK_STATUS_LP_MODE_BUSY  = 0x2000, ///< Link status LP operation busy status
...
typedef struct st_mipi_dsi_cmd
{
    ...
    mipi_dsi_cmd_flags_t flags;        ///< Flags controlling this message transition
...
typedef __PACKED_STRUCT st_mipi_dsi_status_t
{
...
typedef __PACKED_STRUCT st_mipi_dsi_result
{
    ...
    uint8_t           rx_success           : 1; ///< Rexponse packet or ack trigger received
...
typedef struct st_mipi_dsi_timing
{
    ...
    uint32_t clock_beforehand_time;    ///< Clock beforehad time
...
{
    display_instance_t const  * p_display_instance;         ///< Pointer to display source instance
