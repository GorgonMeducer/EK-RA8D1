...
fsp_err_t R_MIPI_DSI_Open (mipi_dsi_ctrl_t * const p_api_ctrl, mipi_dsi_cfg_t const * const p_cfg)
{
    ...
    /* Initialize GLCDC */
    err = p_cfg->p_display_instance->p_api->open(p_cfg->p_display_instance->p_ctrl, p_cfg->p_display_instance->p_cfg);
...
fsp_err_t R_MIPI_DSI_Close (mipi_dsi_ctrl_t * const p_api_ctrl)
{
    ...
    /* Function could hang indefinitely if not open or peripheral is in-use */
    FSP_ERROR_RETURN(MIPI_DSI_OPEN == p_ctrl->open, FSP_ERR_NOT_OPEN);
    FSP_ERROR_RETURN(!(R_DSILINK->LINKSR & R_DSILINK_LINKSR_VRUN_Msk), FSP_ERR_IN_USE);
    ...
    /* Close Display instance - Retry, since close can fail early if certain operations are in progress. */
    display_instance_t const * p_display = p_cfg->p_display_instance;
    fsp_err_t err = p_display->p_api->close(p_display->p_ctrl);
    while ((FSP_SUCCESS != err) && (FSP_ERR_ASSERTION != err) && (FSP_ERR_NOT_OPEN != err))
    {
        /* Keep trying until GLCDC stops successfully or parameter checking is triggered */
        err = p_display->p_api->close(p_display->p_ctrl);
    }
...
fsp_err_t R_MIPI_DSI_Start (mipi_dsi_ctrl_t * const p_api_ctrl)
{
    ...
    /* Start pixel data input input
     *  - Retry if it fails. This is to account for an issue that sometimes happens when closing GLCDC */
    display_instance_t const * p_display = p_cfg->p_display_instance;
    p_display->p_api->start(p_display->p_ctrl);
 ...
 * @retval   FSP_ERR_ASSERTION          p_api_ctrl is NULL.
 * @retval   FSP_ERR_NOT_OPEN           Instance is not open.
 * @retval   FSP_ERR_NOT_ENABLED        Video not running.
 * @retval   FSP_ERR_IN_USE             DSI cannot be closed while ULPS is active.
 * @retval   FSP_ERR_INVALID_MODE       Display returned invalid mode
 **********************************************************************************************************************/
fsp_err_t R_MIPI_DSI_Stop (mipi_dsi_ctrl_t * const p_api_ctrl)
{
    ...
    FSP_ERROR_RETURN(R_DSILINK->VMSR_b.RUNNING, FSP_ERR_NOT_ENABLED);
    ...
    /* Stop GLCDC input - Keep trying until GLCDC stops successfully or an error occurrs */
    fsp_err_t err;
    do
    {
        display_instance_t const * p_display = p_ctrl->p_cfg->p_display_instance;
        err = p_display->p_api->stop(p_display->p_ctrl);
    } while ((FSP_SUCCESS != err) && (FSP_ERR_ASSERTION != err) && (FSP_ERR_INVALID_MODE != err));
 ...
 **********************************************************************************************************************/
static void r_mipi_dsi_call_callback (mipi_dsi_instance_ctrl_t * p_ctrl, mipi_dsi_callback_args_t * p_args)
{
...
void mipi_dsi_seq0 (void) {
    ...
    r_mipi_dsi_call_callback(p_ctrl, &args);
